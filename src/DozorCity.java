...
public class DozorCity extends MIDlet implements CommandListener {
  ...
  public DozorCity() {
  	  ...
  	  busStopNames = new String[9];
  	  busStopUrls = new String[9];
  	  busStopNames[0] = "3 поликл.";
  	  busStopUrls[0] = "t=3&p=25656,39838,24782,25588,25729,24946,27444,25233,25318,26839,43002,25424,26387,26670,26160,26123,26579";
  	  busStopNames[1] = "Мегацентр";
  	  busStopUrls[1] = "t=3&p=25681,39791,24795,25548,25997,24942,27476,25177,25285,26815,43029,25461,26416,26708,26139,26102,26554";
  	  busStopNames[2] = "М.Небабы";
  	  busStopUrls[2] = "t=3&p=25683,24797,25550,25999,25179,25289,26817,43031,26418,26710,26141,26104,26556";
  	  busStopNames[3] = "Боевая";
  	  busStopUrls[3] = "t=3&p=25684,24798,25551,42837,25180,25290,26856,26465,43032,26419,26711,26142,26105,26557";
  	  busStopNames[4] = "Дрозда(кон)";
  	  busStopUrls[4] = "t=3&p=25691,25558,26053,26863,26472,43039,26426,26149,26112,26564";
  	  busStopNames[5] = "Дрозда(нач)";
  	  busStopUrls[5] = "t=3&p=25647,25579,25715,26882,26428,42993,26378,26151,26114,26570";
  	  busStopNames[6] = "Рынок";
  	  busStopUrls[6] = "t=3&p=26940,26996,24793,25370,25981,25174,25283,25135,26813,24237,26755,27497,26720,27184,25459,24193,26312,29048,26100,27165";
  	  busStopNames[7] = "Гер.Чорнобиля";
  	  busStopUrls[7] = "t=3&p=46449,26888,46487,26481,26434";
  	  busStopNames[8] = "Вена";
  	  busStopUrls[8] = "t=3&p=27103,26852,25246,24460,26500,26461,27123";
...
//  	  Thread httpThread = new Thread(new HttpTask(this, "http://paralitix.in.ua/cgi-bin/proxy.py?t=3&p=25681,27707,24795,25548,25997,24942,27476,25177,25285,26815,26416,25461,26708,26139,26102,26554,26513"));
  	  ...
  	  if(httpThread != null) httpThread.start();
  	  	  ...
  	  	  alert.setString("Ошибка создания потока httpThread!");
      ...
      InputStream input = null;
  	  ...
  	  try {
  	  	  ...
  	  	  if(conn == null) {
  	  	  	  ...
  	  	  	  alert.setType(AlertType.INFO);
  	  	  	  alert.setTimeout(Alert.FOREVER);
  	  	  	  alert.setString("Ошибка подключения к серверу!");
  	  	  	  display.setCurrent(alert);
  	  	  	  return null;
  	  	  }
  ...
  public void callback(String value) {
  	  if(value == null){
  	  	  ...
  	  	  alert.setType(AlertType.INFO);
  	  	  alert.setTimeout(Alert.FOREVER);
  	  	  alert.setString("Ошибка при выполнении запроса. Нечего отображать!");
  	  	  display.setCurrent(alert);
  	  	  return;
  	  }
  	  flagDataReady = true;
